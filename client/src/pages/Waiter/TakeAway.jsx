    ...
    return (
        <div className="h-full flex flex-col relative overflow-hidden">
            <div className="flex flex-col lg:flex-row h-full gap-5 animate-fade-in overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-[var(--theme-bg-card)] rounded-3xl border border-[var(--theme-border)] shadow-2xl overflow-hidden">
                        ...
                        <div className="flex items-center gap-2 flex-1 max-w-xl">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full bg-[var(--theme-bg-dark)] text-[var(--theme-text-main)] rounded-xl pl-10 pr-4 py-2 border-none focus:ring-2 focus:ring-blue-500 text-sm"
                                />
                                {searchQuery && (
                    ...
                    <div className="flex-1 flex overflow-hidden">
                        {/* Categories Rail */}
                        <div className="w-20 md:w-32 flex-shrink-0 border-r border-[var(--theme-border)] flex flex-col overflow-y-auto custom-scrollbar bg-black/5">
                            <button
                            ...
                            {categories.map(cat => (
                                <button
                                    ...
                                    style={{
                                        backgroundColor: selectedCategory === cat._id ? `${cat.color || '#3b82f6'}15` : 'transparent',
                                        borderColor: selectedCategory === cat._id ? (cat.color || '#3b82f6') : 'transparent',
                                        color: selectedCategory === cat._id ? (cat.color || '#3b82f6') : 'var(--theme-text-muted)'
                                    }}
                                >
                                    <div
                                        className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors"
                                        style={{
                                            backgroundColor: selectedCategory === cat._id ? (cat.color || '#3b82f6') : 'var(--theme-bg-hover)',
                                            color: selectedCategory === cat._id ? 'white' : 'var(--theme-text-muted)'
                                        }}
                                    >
                                        {cat.name.charAt(0)}
                                    </div>
                                    <span className={`text-[10px] md:text-xs font-bold uppercase tracking-tight text-center leading-tight ${selectedCategory === cat._id ? '' : 'hover:text-[var(--theme-text-main)]'}`}>{cat.name}</span>
                                </button>
                            ))}
                        </div>
            ...
            {cart.length > 0 && !isCartOpen && (
                <div className="lg:hidden fixed bottom-20 left-4 right-4 z-40 bg-blue-600 rounded-2xl shadow-2xl p-4 flex items-center justify-between animate-slide-up">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <ShoppingCart size={24} className="text-white" />
                            <span className="absolute -top-2 -right-2 w-5 h-5 bg-white text-blue-600 rounded-full flex items-center justify-center text-xs font-black">{cart.length}</span>
                        </div>
                    ...
                    <button
                        ...
                        className="bg-white text-blue-600 px-5 py-2.5 rounded-xl font-black text-sm active:scale-95 flex items-center gap-2"
                    >
